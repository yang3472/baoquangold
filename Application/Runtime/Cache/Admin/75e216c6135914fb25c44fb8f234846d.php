                    ...
                    <fieldset>
                        <div style="float: left">
                            <div class="control-group">
                                ...
                                <div class="controls">
                                    <select name="position" id="position">
                                        ...
                                        <?php if(is_array($position_config)): foreach($position_config as $key=>$vo): ?><option value="<?php echo ($vo["id"]); ?>" <?php if($data["position"] == $vo[id] ): ?>selected<?php endif; ?> ><?php echo ($vo["remark"]); ?></option><?php endforeach; endif; ?>
                                    </select>
                                </div>
                            </div>
                            <div class="control-group">
                                <label class="control-label">
                                    <span style="color: red">*</span>图片<br/>
                                </label>
                                <div class="controls">
                                    <input type="file" name="file_upload"  id="file_upload"  multiple="true" >
                                    <div>
                                        <input id="img_url" class="input-xlarge" readonly type="text" value="<?php echo ((isset($data["img_url"]) && ($data["img_url"] !== ""))?($data["img_url"]):''); ?>"  style="width: 350px" >
                                        <span class="btn btn-small btn-danger" style="cursor: pointer" onclick="clearImg('img_url')" >清除</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="control-group">
                                ...
                                <div class="controls">
                                    http://www.baoquangold.com<input class="input-xlarge" value="<?php echo ((isset($data["href_url"]) && ($data["href_url"] !== ""))?($data["href_url"]):''); ?>" id="href_url" placeholder="请输入跳转地址" type="text">
                                </div>
                            </div>
                            ...
                            <div class="control-group">
                                ...
                                <div class="controls">
                                    <input class="input-xlarge" value="<?php echo ((isset($data["title"]) && ($data["title"] !== ""))?($data["title"]):''); ?>" id="title" placeholder="请输入图片标题" type="text">
                                </div>
                            </div>
                            <div class="control-group">
                                <label class="control-label" for="descrip">描述</label>
                                <div class="controls">
                                    <input class="input-xlarge" value="<?php echo ((isset($data["descrip"]) && ($data["descrip"] !== ""))?($data["descrip"]):''); ?>" id="descrip" placeholder="请输入描述" type="text">
                                </div>
                            </div>
                            <div class="control-group">
                                <label class="control-label" for="order_sort"><span style="color: red">*</span>排序</label>
                                <div class="controls">
                                    <input class="input-xlarge" value="<?php echo ((isset($data["order_sort"]) && ($data["order_sort"] !== ""))?($data["order_sort"]):''); ?>" id="order_sort" placeholder="请输入排序" type="text">（数字越小越靠前）
                                </div>
                            </div>
                        </div>
                        <div style="float: right">
                            <h3>位置示例：</h3>
                            <?php if(is_array($position_config)): foreach($position_config as $key=>$vo): if($vo["img"] != '' ): ?><div><?php echo ($vo["remark"]); ?><a href="<?php echo ($vo["img"]); ?>" target="_blank" style="margin-left: 10px;color: red" >点击查看</a></div><?php endif; endforeach; endif; ?>
                        </div>
